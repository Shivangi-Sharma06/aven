...
export default function AvenNetwork() {
  ...
  useGSAP(
    () => {
      ...
      const links = gsap.utils.toArray('[data-link]', root) as any[]
      const nodes = gsap.utils.toArray('[data-node]', root) as any[]
      const particles = gsap.utils.toArray('[data-particle]', root) as any[]
      const depthLayers = gsap.utils.toArray('[data-depth-layer]', root) as any[]
      ...
      const strongLinks = gsap.utils.toArray("[data-strong='true']", root) as any[]
      ...
      let hasMeasuredInitialState = false
      const triggerFrame = requestAnimationFrame(() => {
        ...
        heroTrigger = ScrollTrigger.create({
          ...
          onToggle: ({ isActive }) => {
            if (!hasMeasuredInitialState) {
              hasMeasuredInitialState = true
              return
            }
    ...
    <div ref={rootRef} className="aven-network-wrap" aria-hidden="true">
      ...
      >
        ...
        <g data-depth-layer="0.45" className="network-depth network-depth--far">
          ...
          <g>
            {networkPaths.filter((_, index) => index % 3 === 0).map(([id, d, strong]: any) => (
              <path
                key={id}
                id={id}
                d={d}
        ...
        <g data-depth-layer="0.8" className="network-depth network-depth--mid">
          ...
          <g>
            {networkPaths.filter((_, index) => index % 3 === 1).map(([id, d, strong]: any) => (
              <path
                key={id}
                id={id}
                d={d}
        ...
        <g data-depth-layer="1.25" className="network-depth network-depth--near">
          ...
          <g>
            {networkPaths.filter((_, index) => index % 3 === 2).map(([id, d, strong]: any) => (
              <path
                key={id}
                id={id}
                d={d}
